...
//  n_slack_ = n_ub_ + n_bounds_;
  ...
  for (size_t i = 0; i < n_dim_; ++i) {
    ...
    switch (x_type_[i]) {
      case StateType::NORMAL:lp_next.c->coeffRef(i + current_shift) = prog.c->coeff(i);
        if (prog.b_eq)
        ...
        break;
      case StateType::NEGATIVE:lp_next.c->coeffRef(i + current_shift) = -prog.c->coeff(i);
        if (prog.b_eq)
          for (Eigen::SparseMatrix<double>::InnerIterator it(*prog.A_eq, i); it; ++it)
            lp_next.A_eq->insertBack(it.row(), i + current_shift) = -it.value();
        if (prog.b_ub || n_bounds_)
          for (Eigen::SparseMatrix<double>::InnerIterator it(*prog.A_ub, i); it; ++it)
            lp_next.A_ub->insertBack(n_bounds_ + it.row(), i + current_shift) = -it.value();
        break;
      case StateType::COMPACT:lp_next.c->coeffRef(i + current_shift) = prog.c->coeff(i);
        ...
        if (prog.b_ub || n_bounds_)
          for (Eigen::SparseMatrix<double>::InnerIterator it(*prog.A_ub, i); it; ++it)
            lp_next.A_ub->insertBack(n_bounds_ + it.row(), i + current_shift) = it.value();
        break;
      case StateType::UNBOUNDED:lp_next.c->coeffRef(i + current_shift) = prog.c->coeff(i);
        if (prog.b_eq)
          for (Eigen::SparseMatrix<double>::InnerIterator it(*prog.A_eq, i); it; ++it)
            lp_next.A_eq->insertBack(it.row(), i + current_shift) = it.value();
        if (prog.b_ub || n_bounds_)
...
void stuka::LP::BoundsZero::setObjective(const std::unique_ptr<Eigen::VectorXd> &c) {

}

void stuka::LP::BoundsZero::setRHS(const std::unique_ptr<Eigen::VectorXd> &b_ub,
                                   const std::unique_ptr<Eigen::VectorXd> &b_eq) {

}

void stuka::LP::BoundsZero::setBounds(const std::unique_ptr<Eigen::VectorXd> &lb,
                                      const std::unique_ptr<Eigen::VectorXd> &ub) {

}

void
stuka::LP::BoundsZero::addVar(double c, const std::unique_ptr<Eigen::VectorXd> &a_ub, const std::unique_ptr<Eigen::VectorXd> &a_eq,
                              double lb, double ub) {

}

void
stuka::LP::BoundsZero::addVars(const std::unique_ptr<Eigen::VectorXd> &c, const std::unique_ptr<Eigen::SparseMatrix<double>> &A_ub,
...
void stuka::LP::BoundsZero::addConstr_ub(const std::unique_ptr<Eigen::VectorXd> &a, const double &b) {

}

void stuka::LP::BoundsZero::addConstrs_ub(const std::unique_ptr<Eigen::SparseMatrix<double>> &A,
...
void stuka::LP::BoundsZero::addConstr_eq(const std::unique_ptr<Eigen::VectorXd> &a, const double &b) {

}

void stuka::LP::BoundsZero::addConstrs_eq(const std::unique_ptr<Eigen::SparseMatrix<double>> &A,
                                          const std::unique_ptr<Eigen::VectorXd> &b) {
...
Eigen::VectorXd stuka::LP::BoundsZero::revertState(const Eigen::VectorXd &x) {
  ...
  for (size_t i = 0; i < n_dim_; ++i) {
    switch (x_type_[i]) {
      case StateType::COMPACT:
      case StateType::NORMAL:x_.coeffRef(i) += xn.coeff(i + current_shift);
        break;
      case StateType::NEGATIVE :x_.coeffRef(i) -= xn.coeff(i + current_shift);
        break;
      case StateType::UNBOUNDED :x_.coeffRef(i) += xn.coeff(i + current_shift) - xn.coeff(i + current_shift + 1);
        current_shift += 1;
        break;
    }
  }
