...
stuka::QP::QuadraticProgram stuka::example::SimpleQP::gen() {
  ...
  prog.Q = std::make_unique<Eigen::SparseMatrix<double>>(2, 2);
  prog.Q->coeffRef(0, 0) = 1;
  prog.Q->coeffRef(0, 1) = -1;
  prog.Q->coeffRef(1, 0) = -1;
  prog.Q->coeffRef(1, 1) = 2;

  prog.c = std::make_unique<Eigen::VectorXd>(2);
  *prog.c << -2, -6;

  prog.A_ub = std::make_unique<Eigen::SparseMatrix<double>>(3, 2);
  ...
  prog.b_ub = std::make_unique<Eigen::VectorXd>(3);
  *prog.b_ub << 2, 2, 3;

  prog.lb = std::make_unique<Eigen::VectorXd>(2);
  *prog.lb << 0, 0;

  prog.ub = std::make_unique<Eigen::VectorXd>(2);
