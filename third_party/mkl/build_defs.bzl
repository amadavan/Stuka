...
def if_mkl(if_true, if_false = []):
    ...
    """
    return select({
        str(Label("@stuka//third_party/mkl:enable_mkl")): if_true,
...
def mkl_deps():
    ...
    """
    return select({
        str(Label("@stuka//third_party/mkl:enable_mkl")): ["@stuka//third_party/mkl:intel_binary_blob"],
        "//conditions:default": [],
    })

def _enable_local_mkl(repository_ctx):
    return _MKL_ROOT in repository_ctx.os.environ
...
def _mkl_autoconf_impl(repository_ctx):
    ...
    if _enable_local_mkl(repository_ctx):
        ...
        mkl_root = repository_ctx.os.environ[_MKL_ROOT]
